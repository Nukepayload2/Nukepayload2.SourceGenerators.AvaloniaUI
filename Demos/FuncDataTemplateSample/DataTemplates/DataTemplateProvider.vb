...
Namespace DataTemplates
	...
	Public Module DataTemplateProvider
		' This FuncDataTemplate can be static, as it will not change over time.
		Public ReadOnly Property GenderDataTemplate As New FuncDataTemplate(Of Person)(
			Function(person) person IsNot Nothing, AddressOf BuildGenderPresenter)

		' This private function will return a control that represents our persons sex as a gender symbol.
		Private Function BuildGenderPresenter(person As Person) As Control
			...
			path(ToolTip.TipProperty) = New Binding(NameOf(person.Sex))
			...
			Select Case person.Sex
				...
				Case Else
					' Fall-back value
					Return New TextBlock With {.Text = "NOT SUPPORTED"}
			End Select
